...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    val suggestions: Observable[Try[List[String]]] = {
      searchTerms.concatRecovered { x => wikiSuggestResponseStream(x).timedOut(5) }
    }
    ...
    // TO IMPLEMENT
    val selections: Observable[String] = button.clicks.map { x => suggestionList.selection.items.head }

    // TO IMPLEMENT
    val pages: Observable[Try[String]] = selections.concatRecovered { x =>
      wikiPageResponseStream(x).timedOut(5)
    }
